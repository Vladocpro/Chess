          ...
          <div className={'flex flex-col gap-3'}>
             {invitations?.map((invitation) => (
                 <div className={'flex items-center justify-between gap-5'} key={invitation._id}>
                    <Link className={'flex flex-[1] items-center gap-2 sm:gap-5'} to={`/profile/${invitation.sender.username}`}>
                       <ProfileIcon size={'lg'} textValue={invitation.sender.username} isMyProfile={false} iconStyles={'w-[2.25rem] h-[2.25rem] text-lg sm:w-[4rem] sm:h-[4rem] sm:text-3xl'} />
                       <div className={'flex flex-col'}>
                          <span>{invitation.sender.username}</span>
                          <span className={'text-sm hidden sm:inline-block'}>Joined {moment(invitation.sender.createdAt).format('ll')}</span>
                       </div>
                    </Link>
