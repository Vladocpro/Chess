       ...
       <div className={'bg-primary py-5 px-5 rounded-md'}>
          ...
          <div className={'flex flex-col mt-3 gap-2'}>
             {friends.slice(0, 5).sort((a, b) => b.rating - a.rating).map((friend, index) => (
                 <Link className={'flex items-center gap-2 text-gray-300 hover:text-white cursor-pointer'}
                       to={`/profile/${friend._id}`}>
                    <span className={'w-6'}>#{index + 1}</span>
                    <ProfileIcon size={'sm'} withText={true} isMyProfile={false} textValue={friend.username}
                                 textStyles={'max-w-[143px] truncate'}/>
