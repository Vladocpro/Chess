...
const OpponentSearch: FC<OpponentSearchProps> = ({chosenOpponent, setChosenOpponent}) => {
   ...
   return (
       <div className={'relative'}  ref={dropdownRef}>
          <div className={`flex gap-3 items-center  cursor-pointer select-none justify-between h-10 sm:h-12 ${searchValue.length > 0 ? 'rounded-t-lg ' : 'rounded-lg'} bg-primaryLight`}>
             ...
             <div className="relative w-full">

                {
                   chosenOpponent === null ?
                       ...
                       <div className={`absolute left-2 top-2 h-[2rem] w-[2rem] flex items-center justify-center text-lg bg-primary rounded-full`}>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} onClick={() => setChosenOpponent(null)}  className={`absolute right-2 top-2 fill-white h-8 w-8 stroke-primaryLight ${chosenOpponent === null ? 'hidden' : 'block'}`}>
             ...
             <div
                ...
                {users.slice(0,5).map((opponent, index) => (
                    <div
                       ...
                       <ProfileIcon size={'sm'} withText={true} isMyProfile={false} textValue={opponent.username} textStyles={'max-w-[143px] truncate'} bgColor={'#2A3033'} iconStyles={'h-[2rem] w-[2rem] text-lg'}/>
