...
const Club = () => {
   ...
   const handleLeaveClub = () => {
      postFetch(`/clubs/leave`, {clubID: id})
          .then((response) => {
             user.setClub(null)
             setClub({...club, membersCount: club.membersCount -1})
             openToast({message: response, type: ToastType.SUCCESS, position: ToastPositions.AUTH, duration: 1000})
          })
          .catch((error) => {
             console.log(error)
          })
   ...
   const handleJoinClub = () => {
      postFetch(`/clubs/join`, {clubID: id})
          .then((response) => {
             user.setClub(response.club)
             setClub({...club, membersCount: club.membersCount + 1})
             openToast({message: response.message, type: ToastType.SUCCESS, position: ToastPositions.AUTH, duration: 1000})
          })
          .catch((error) => {
             console.log(error)
          })
          ...
          <div className={'bg-primary py-5 px-5 rounded-md gap-5 flex-[2] max-w-[900px]'}>
            <div className={'flex flex-col'}>
               <div className={'flex gap-5'}>
                  <img  className={'w-[2.25rem] h-[2.25rem] sm:w-[5rem] sm:h-[5rem] '} src={'/club.png'} alt={'chess club'}/>
                  <div>
                     <span className={'text-2xl'}>{club?.clubname}</span>
                     <div className={'flex gap-7 mt-2'}>
                        <div className={'flex gap-3 '}>
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"  className="w-6 h-6 fill-gray-400">
               ...
               <div className={'mt-4'}>
                  {/* TODO show red leave button if user is member of this club, otherwise green join club */}
                  {
                     user.club?._id?.toString() === id ?
                         (
                             <button onClick={handleLeaveClub} className={'text-sm sm:text-base bg-primaryLight hover:bg-red-700 duration-200 rounded-md py-2 sm:py-3 px-4 sm:px-7 min-w-[145px] w-1/3'} >Leave Club</button>
                         ) : (
                             <button onClick={handleJoinClub} className={'text-sm sm:text-base bg-primaryLight hover:bg-secondaryGreen duration-200 rounded-md py-2 sm:py-3 px-4 sm:px-7 min-w-[145px] w-1/3'} >Join Club</button>
